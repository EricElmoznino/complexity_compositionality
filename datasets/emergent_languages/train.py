...
lis_optimizer = OPTIMISER(
    listener.parameters(), lr=LEARNING_RATE * DECODER_LEARING_RATIO
)
...
def train_phaseB(
    ...
    lis_entropy = -(
        F.softmax(pred_vector, dim=1) * F.log_softmax(pred_vector, dim=1)
    ).sum(dim=1)
    lis_log_prob = F.log_softmax(pred_vector.max(dim=1)[0], dim=0)
    pred_idx = F.softmax(pred_vector, dim=1).argmax(dim=1)
    ...
    lis_loss = -(
        (reward_vector.detach() * lis_log_prob).mean()
        + 0.05 * exp_ratio * lis_entropy.mean()
    )
    lis_loss.backward()

    if MSG_MODE == "REINFORCE":
        spk_loss = -(
            (reward_vector.detach() * spk_log_prob).mean()
            + 0.1 * exp_ratio * spk_entropy.mean()
        )
        spk_loss.backward()
    elif MSG_MODE == "SCST":
        speaker.eval()
        ...
        spk_loss = -(
            ((reward_vector.detach() - reward_vector_.detach()) * spk_log_prob).mean()
            + 0.1 * spk_entropy.mean()
        )
        spk_loss.backward()
    ...
    if update == "BOTH":
        ...
        print(
            'Please input "BOTH", "SPEAKER" or "LISTENER" for the train_epoch function'
        )
...
def train_phaseC(
    speaker, listener, train_batch, train_candidates, sel_idx_train, rwd_filter=False
):
    """
    ...
    with torch.no_grad():
        speaker.train(
            True
        )  # Here we use train model because we want to sample from posterior, not argmax
        ...
        if rwd_filter == True:
            ...
            msg = msg.transpose(
                0, 1
            )  # Change the size to [N_B, ATTRI_SIZE, MSG_VOCSIZE]
            new_msg = []
            for i in range(rewards.shape[0]):
                if (
                    rewards[i] == 1
                ):  # If this obj-msg pair can correctly play the game, use this pair
...
for i in range(args.max_gen):
    ...
    while Ig_cnt < args.Ig:
        ...
        if Ig_cnt % 4 == 1:
            ...
            train_batch, train_candidates, sel_idx_train = (
                batch_list[0]["data"],
                batch_list[0]["candidates"],
                batch_list[0]["sel_idx"],
            )

        if Ig_cnt <= args.Ib:
            reward, spk_loss, lis_loss = train_phaseB(
                speaker,
                listener,
                spk_optimizer,
                lis_optimizer,
        ...
        if Ig_cnt % 500 == 1:
            print(
                "Gen.%d ==PhaseB==Round %d, rwd (%d, %d), spk_loss %.4f, lis_loss %.4f"
                % (i, Ig_cnt, reward, rwd_avg20, spk_loss, lis_loss)
            )
    ...
    print(
        "Gen.%d @@PhaseA@@, round is %d, acc is %.4f, acc_avg20 is %.4f"
        % (i, Ia_cnt, acc, acc_avg20)
    )
    print(comp_ps[-1])
...
if not os.path.exists(args.path):
    os.mkdir(args.path)
np.save(os.path.join(args.path, "comp_ps.npy"), comp_ps)
...
torch.save(w, os.path.join(args.path, "w.pt"))
torch.save(z, os.path.join(args.path, "z.pt"))

if WANDB_ENTITY != "":
    wandb.init(
        entity=WANDB_ENTITY,
        project="Complexity - Emergent Languages (data generation)",
        dir=args.path,
        config={
            "resets": RESETS,
            "num_object_attributes": ATTRI_SIZE,
            "num_attribute_vals": NUM_SYSTEM,
            "message_length": MSG_MAX_LEN,
            "vocab_size": MSG_VOCSIZE,
            "msg_mode": MSG_MODE,
            "seed": args.seed,
            "lr": LEARNING_RATE,
            "lr_ratio": DECODER_LEARING_RATIO,
            "batch_size": BATCH_SIZE,
            "clip": CLIP,
            "num_pretrain_rounds_speaker": args.Ia,
            "num_pretrain_rounds_listener": args.Ib,
            "num_interaction_rounds": args.Ig,
            "valid_num": VALID_NUM,
            "max_generations": args.max_gen,
            "pairs_teach": args.pairs_teach,
        },
    )
