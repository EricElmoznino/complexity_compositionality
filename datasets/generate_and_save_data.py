...
for vocab_size in [1000]:
    for d in [256, 512, 1024]:
        for k in [1, 2, 4, 8, 16, 32, 64]:
            ...
            torch.save(torch.tensor(w_train), f'{data_folder_path}/{key_string}/w_train.pt')
            torch.save(torch.tensor(z_train), f'{data_folder_path}/{key_string}/z_train.pt')
            torch.save(torch.tensor(w_test), f'{data_folder_path}/{key_string}/w_test.pt')
            torch.save(torch.tensor(z_test), f'{data_folder_path}/{key_string}/z_test.pt')
